...
import { Menu, X, CalendarCheck, FileText } from 'lucide-react'
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white border-b-4 border-black py-3 md:py-4">
      <div className="section-container flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-6">
          {navLinks.map((link) => (
            <Link
              ...
              href={link.href}
              className={cn(
                "text-sm font-black uppercase tracking-widest transition-colors p-2 whitespace-nowrap",
        ...
        <div className="flex items-center gap-4 shrink-0">
          <a href="https://cal.com/brylcodes/30min" target="_blank" rel="noopener noreferrer" className="hidden md:flex neo-btn bg-white text-black hover:bg-neo-secondary transition-colors px-4 py-2 text-xs items-center gap-2 whitespace-nowrap shrink-0">
            <CalendarCheck className="w-4 h-4" strokeWidth={3} /> BOOK A SYNC
          </a>
          <Link href="/contact" className="hidden sm:block neo-btn bg-neo-accent text-white hover:bg-black transition-colors px-4 py-2 text-xs md:text-sm md:px-6 md:py-3 whitespace-nowrap shrink-0">
            GET STARTED
          </Link>
